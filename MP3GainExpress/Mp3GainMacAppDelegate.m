...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    NSMenuItem *languageMenuItem = [[NSMenuItem alloc] init];
    languageMenuItem.title = NSLocalizedString(@"Language menu", @"Language");
    NSMenu *languageMenu = [[NSMenu alloc] initWithTitle:NSLocalizedString(@"Language menu", @"Language")];
    [languageMenuItem setSubmenu:languageMenu];
    NSMenuItem *selectLanguageItem = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Select Language menu item", @"Select Language...")
                                                                action:@selector(showLanguageSelector:)
                                                         keyEquivalent:@"l"];
    selectLanguageItem.keyEquivalentModifierMask = NSEventModifierFlagCommand;
    selectLanguageItem.target = self;
    [languageMenu addItem:selectLanguageItem];
    [mainMenu insertItem:languageMenuItem atIndex:mainMenu.numberOfItems - 1];
}
...
- (IBAction)showLanguageSelector:(id)sender {
    [[M3GLanguageSelectorController sharedController] showForWindow:_window];
}
